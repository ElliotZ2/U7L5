...
public class RepairSchedule
{
    ...
    {
        for(CarRepair carRep : schedule) {
            if(carRep.getBayNum() == b || carRep.getMechanicNum() == m) {
                return false;
            }
        }
    ...
    public ArrayList<Integer> availableMechanics()
    {
        ...
        for(int i = 0; i < schedule.size(); i++) {
            int occupiedMech = schedule.get(i).getMechanicNum();
            for(int j = 0; j < availableMechs.size(); j++) {
                if(occupiedMech == availableMechs.get(j)) {
